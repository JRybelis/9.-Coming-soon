import { isEmailValid, isNameValid, isTextValid } from "./validationRules.js"
    ...
    submitBtnDOM.addEventListener("click", () => {
        ...
        for (let input of allElements) {
            ...
            if (validationRule === 'name') {
                const nameError = isNameValid(text);
                if (nameError !== true) {
            ...
            if (validationRule === 'email') {
                const emailError = isEmailValid(text);
                if (emailError !== true) {
            ...
            if (validationRule === "text") {
                const textError = isTextValid(text);
                if (textError !== true) {
